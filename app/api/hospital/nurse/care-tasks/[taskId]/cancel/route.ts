...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    if (task.status === "Cancelled") return errorResponse("Task is already cancelled", 409);
    if (task.status === "Completed") return errorResponse("Cannot cancel a completed task", 409);

    task.status = "Cancelled";
    if (body.cancellationReason) task.completionNotes = body.cancellationReason;
    await task.save();

    return successResponse(serializeDoc(task), "Care task cancelled");
  } catch (error) {
