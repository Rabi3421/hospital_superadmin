...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const hospital = await Hospital.findOneAndUpdate(
      { hospitalId },
      { status: "Cancelled", websiteStatus: "Maintenance" },
      { new: true },
    );
    if (!hospital) return errorResponse("Hospital not found", 404);

    await Subscription.findOneAndUpdate({ hospitalId }, { status: "Cancelled" });
    return successResponse(serializeDoc(hospital), "Hospital cancelled");
  } catch (error) {
